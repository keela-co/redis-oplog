...
Object.assign(ObserveMultiplexer.prototype, {
    ...
    removeHandle: function(id) {
        ...
        if (!self._ready())
            throw new Error(
                "Can't remove handles until the multiplex is ready"
            );
    ...
    onFlush: function(cb) {
        ...
        self._queue.queueTask(function() {
            if (!self._ready())
                throw Error(
                    'only call onFlush on a multiplexer that will be ready'
                );
            cb();
        });
    },
    ...
    _applyCallback: function(callbackName, args) {
        ...
        self._queue[runType](function() {
            const time = Date.now() - queueStartTime;

            if (Config.signupSessionsDebug) {
                console.log(JSON.stringify({ callbackName, time, args, message: '[ObserveMultiplex] Queue wait time' }));
            }
            ...
            // though.
            self._cache.applyChange[callbackName].apply(
                null,
                EJSON.clone(args)
            );
            ...
            if (Config.signupSessionsDebug) {
                const completionTime = Date.now() - queueStartTime;
                console.log(JSON.stringify({ callbackName, time: completionTime , args, message: '[ObserveMultiplex] completion time' }));
            }
        });
    },
